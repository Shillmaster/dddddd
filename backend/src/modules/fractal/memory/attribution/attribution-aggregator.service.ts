...
export class AttributionAggregatorService {
   ...
   */
  getDateRange(windowDays: number): { from: string; to: string } {
    const to = new Date();
    const from = new Date();
  ...
  async getOutcomes(
    ...
    const { from, to } = this.getDateRange(windowDays);
    
    return PredictionOutcomeModel.find({
      symbol,
      preset,
      role,
      asofDate: { $gte: from, $lte: to }
    }).lean() as Promise<PredictionOutcomeDocument[]>;
  }
  ...
  async getAttributionData(
    ...
    // Fetch outcomes
    const outcomes = await this.getOutcomes(symbol, windowDays, preset, role);
